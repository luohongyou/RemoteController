...
*新功能与改进
主页的卡片现在得到了更新，更加美观且信息更丰富
文件浏览现已升级为文件管理，大大完善了文件浏览界面，并新增支持对文件的各种管理操作
添加了新的“共享云盘”并支持免账户使用PIN登录
新增了执行命令工具，支持以各种方式运行Windows批处理脚本
基础控制中现增加了屏幕锁定模块，支持锁定远程计算机屏幕
屏幕截图现升级为实时屏幕，支持动态实时屏幕功能（Beta版）
系统设置模块新增了多个设置项，更便于管理员进行管理
程序更新模块得到了完善，支持显示上传进度并不再卡死
用户管理的相关功能得到完善，现在能查看用户的更多信息
托盘图标的功能现在得到完善和增强，现在可以从托盘图标直接打开相关管理界面
*Bug修复
