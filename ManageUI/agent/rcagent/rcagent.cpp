        ...
        while (true)
        {
            ...
            else
            {
                ...
                if (Action == "printscreen")
                {
                    ...
                    std::stringstream tempIO;
                    tempIO << "Screenshoot-" << User << '-' << StandardTime(true);
                ...
                else if (Action == "onewaymessage")
                {
                    ...
                    ShellExecuteA(NULL, "open", "..\\..\\ManageUI\\messager\\onewaymessager.exe", (Type + " " + Method + " " + Title + " " + Content).c_str(), NULL, SW_SHOW);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NOTIFYICON:
        ...
        else if ((wParam == IDI_ICON1) && (lParam == WM_RBUTTONDOWN))
        {
            POINT pt;
            GetCursorPos(&pt);
