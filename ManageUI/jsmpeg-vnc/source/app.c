...
int app_on_http_req(app_t *self, libwebsocket *socket, char *request) {
	...
	else if (strcmp(request, "/main.html") == 0) { // TODO: Request Connection Key from Auth Manager
		libwebsockets_serve_http_file(self->server->context, socket, "client/main.html", "text/html; charset=utf-8", NULL);
		return true;
	}
